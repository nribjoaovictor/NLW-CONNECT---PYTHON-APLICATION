...
def create_new_subs():
    ...
    httpResponse=subs_creator.create(http_request)

    return jsonify(httpResponse.body),httpResponse.status_code
...
def subscribers_by_link(link, event_id):    
    ...
    httpResponse=subs_manager.get_subscribers_by_link(http_request)
...
def link_ranking(event_id):    
    ...
    httpResponse=subs_manager.get_event_ranking(http_request)
